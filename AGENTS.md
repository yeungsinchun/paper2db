...
- LQ Book 5 listings go through `apply_book5_listings()` in `scripts/classify_lq_keywords.py` after either classify-lq backend (keyword or LLM); `tests/test_lq_book5_sections.py` pins the per-chapter problem list from OCR fixtures under `tests/fixtures/lq_ocr/`. Section `questions.pdf` include non-primary listings, so a stale PDF means the section-pdfs stage was not re-run.
