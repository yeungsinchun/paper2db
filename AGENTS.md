...
- CI (`.github/workflows/pipeline-release.yml`, README "CI and releases") does the full build with `--replay-classifications` (tracked JSON only, never LLM or keyword fallback), fails if any tracked file changes or a generated-bank test skips, and publishes `build-<branch>-<sha7>` / `latest-<branch>` prereleases. Without replay or an LLM key, classify stages fall back to keywords and rewrite `metadata/*/llm_classifications.json`. Avoid `--force` on `lq-pages` unless re-deriving page ranges: it regenerates tracked `starts.json`.
